...
public:
    ...
    static bool overflowed() { return _overflowBit > 0; }

private:
    UnsignedVector _binRepr;
    static Unsigned _overflowBit;
};

template<size_t N>
typename BigUnsignedInt<N>::Unsigned BigUnsignedInt<N>::_overflowBit = 0;
...
std::istream& operator>>(std::istream& is, BigUnsignedInt<N>& bigUnsignedInt)
{
    ...
    //deque<BigUnsignedInt::Unsigned> digits;
    ...
    typename BigUnsignedInt<N>::UnsignedVector::size_type counter = N - 1;
    //deque<BigUnsignedInt::Unsigned>* quotPtr = &quotient;
    //deque<BigUnsignedInt::Unsigned>* divPtr = &digits;
...
std::ostream& operator<<(std::ostream& os, const BigUnsignedInt<N>& bigUnsignedInt)
{
    typename BigUnsignedInt<N>::UnsignedVector::size_type counter = 1;
...
BigUnsignedInt<N> operator+(const BigUnsignedInt<N>& a, const BigUnsignedInt<N>& b)
{
    BigUnsignedInt<N>::_overflowBit = 0;
    BigUnsignedInt<N> result;
    for (typename BigUnsignedInt<N>::UnsignedVector::size_type i = N - 1;
         i != std::numeric_limits<typename BigUnsignedInt<N>::UnsignedVector::size_type>::max(); --i)
    {
        result._binRepr[i] = a._binRepr[i] ^ b._binRepr[i] ^ BigUnsignedInt<N>::_overflowBit;
        BigUnsignedInt<N>::_overflowBit =
                (a._binRepr[i] & b._binRepr[i]) | (BigUnsignedInt<N>::_overflowBit & (a._binRepr[i] ^ b._binRepr[i]));
    }
    return result;
}
...
BigUnsignedInt<N> operator-(const BigUnsignedInt<N>& a, const BigUnsignedInt<N>& b)
{
         ...
         i != std::numeric_limits<typename BigUnsignedInt<N>::UnsignedVector::size_type>::max(); --i)
    {
        ...
        BigUnsignedInt<N>::_overflowBit =
                ...
                (BigUnsignedInt<N>::_overflowBit & ((1 ^ a._binRepr[i]) ^ b._binRepr[i]));
    }
    return result;
}
